...
class CosmosConversationClient():
    ...
    async def get_messages(self, user_id, conversation_id):
        ...
        query = f"SELECT * FROM c WHERE c.conversationId = @conversationId AND c.type='message' AND c.userId = @userId AND (c.isPrompt != true OR NOT IS_DEFINED(c.isPrompt))  ORDER BY c.timestamp ASC"
        messages = []
        async for item in self.container_client.query_items(query=query, parameters=parameters):
